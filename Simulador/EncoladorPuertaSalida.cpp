...
class EncoladorPuertaSalida{
    ...
    public:
        EncoladorPuertaSalida(int nroPuerta){
            ...
            if( (colaSalidaNormal = msgget(ftok(DIRECTORIO_IPC,PUERTA_SALIDA_FILA_NORMAL),PERMISOS)) == -1){
            ...
            SemaforoPuertaEsperando = getsem(MUTEX_PUERTA_ESPERANDO+(nroPuerta-1)*DESP);
            mutexColas = getsem(MUTEX_CONTADOR_COLAS_PUERTAS+(nroPuerta-1)*DESP);
