...
int main (int argc, char** argv){
    ...
    if(argc<3){
        printf("Mal uso 1:id 2:ftok cola entrada 3: ftok cola salida");
    }
    
    Logger::startLog(LOGGER_PATH,ID);//TODO CAMBIAR
    
    int portCS = PORT_FIJO;//TODO cambiar a un port leido desde algun lado
    int portCE = PORT_FIJO+1;
    
    //pido unirme al broker
    int newsockfdCS = tcpopact(name,portCS);
    if (newsockfdCS < 0) {
    ...
    if ((childpid=fork())<0){
        ...
        close(newsockfdCE);
        execlp(PATH_CLIENTE_CS,NAME_CLIENTE_CS,argv[3],CS,tokill,(char*)NULL);
    ...
    if ((childpid=fork())<0){
        ...
        close(newsockfdCS);
        execlp(PATH_CLIENTE_CE,NAME_CLIENTE_CE,argv[1],argv[2],CE,tokill,(char*)NULL);
        Logger::loggError("Error al cargar la imagen de ejecutable del CSCliente");
        exit(1);
    }
}
